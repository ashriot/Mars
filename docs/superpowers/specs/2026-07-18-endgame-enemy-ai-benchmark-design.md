...
- all living allies;
- seeded valid hero;
- the complete eligible hero candidate pool for authored random-hit actions;
...
The battle scene accepts explicit roster, enemy-level, enemy-HP, seed, and reward-policy overrides. Ordinary gameplay continues using `RunManager.party_roster`, the current dungeon tier, an enemy HP multiplier of 1, the current run seed, and ordinary victory rewards when overrides are absent. The lab defaults enemies to Rank 20 with 10 times HP, exposes inspector ranges from Rank 1 through 30 and from 1 through 20 times HP, uses an explicit four-drone encounter, disables run rewards, and creates a battle-local random stream from its fixed seed so CT head starts, critical rolls, random-hit targets, and AI ties are reproducible without mutating the ordinary global random stream. The HP multiplier remains independent of enemy rank and changes only maximum and starting HP, leaving attack, speed, aim, and other rank-scaled stats untouched.

Running the lab must not change singleton save or run data even after victory or defeat. Omitting `GameManager` is not sufficient because `BattleManager` currently awards victory XP directly; the lab setup explicitly disables that write. Benchmark result presentation may report locally but does not award XP, Bits, loot, injuries, or persistent progression.
