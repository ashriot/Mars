...
The battle scene accepts explicit roster, enemy-level, enemy-HP, seed, and reward-policy overrides. Ordinary gameplay continues using `RunManager.party_roster`, the current dungeon tier, an enemy HP multiplier of 1, the current run seed, and ordinary victory rewards when overrides are absent. The lab defaults enemies to Rank 20 with 2.5 times HP, exposes inspector ranges from Rank 1 through 30 and from 1 through 3 times HP, uses an explicit encounter, disables run rewards, and creates a battle-local random stream from its fixed seed so CT head starts, critical rolls, random-hit targets, and AI ties are reproducible without mutating the ordinary global random stream. The HP multiplier remains independent of enemy rank and changes only maximum and starting HP, leaving attack, speed, aim, and other rank-scaled stats untouched.
