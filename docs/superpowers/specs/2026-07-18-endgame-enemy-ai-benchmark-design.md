...
The battle scene accepts explicit roster, enemy-level, seed, and reward-policy overrides. Ordinary gameplay continues using `RunManager.party_roster`, the current dungeon tier, the current run seed, and ordinary victory rewards when overrides are absent. The lab defaults enemies to Rank 30, exposes an inspector range from Rank 1 through 30, uses an explicit encounter, disables run rewards, and creates a battle-local random stream from its fixed seed so CT head starts, critical rolls, random-hit targets, and AI ties are reproducible without mutating the ordinary global random stream.
