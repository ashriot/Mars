...
Scaling rules that consume living-combatant or other battle-manager-owned facts declare that dependency through `requires_battlefield_context()`. Target presence alone does not make those previews complete. Sequential preview derives later current-hit counts from its copied defeat state while leaving the live battlefield unchanged. A random multihit preview with current-hit battlefield scaling becomes incomplete when a possible pre-final defeat can redirect a later hit, because the compact preview does not exhaustively branch across random allocation paths; presentation then uses the authored relationship instead of a false exact range.
