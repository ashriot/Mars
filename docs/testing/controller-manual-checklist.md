...
- [ ] Inside party management with controller ownership, exactly one `128x128` software cursor slightly overlaps the focused target's lower-right edge, stays fully inside the viewport without crossing readable content, and completes each directional movement in 70 ms without delaying logical focus.
- [ ] While hub focus is stationary, the cursor drifts about 12 px diagonally away over 650 ms and returns over 160 ms; the motion pauses during focus travel and never continues under mouse, keyboard, modal, or scan ownership.
