...
- [ ] Inside party management with controller ownership, exactly one `64x64` east-pointing hand sits at the focused target's upper-left edge with its fingertip overlapping by about 8 px, stays fully inside the viewport without crossing readable content, and completes each directional movement in 70 ms without delaying logical focus.
- [ ] While hub focus is stationary, the hand drifts about 6 px left over 650 ms and returns over 160 ms; the motion pauses during focus travel and never continues under mouse, keyboard, modal, or scan ownership.
...
- [ ] Every hero card shows readable exact HP plus compact hero-wide XP with no separators from `0` through `9999`, an 8 px HP/XP gap, and visible right padding; after a successful progression purchase, compact hero XP and the expanded role header's exact comma-formatted `AVAILABLE XP` both refresh while collapsed roles show no XP readout.
