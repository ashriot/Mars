...
func before_each() -> void:
	...
	catalog._roles[loaded.tree.role_id] = loaded.tree
	rebuild_count = 0


func _service() -> ProgressionService:
	return ProgressionService.new(catalog, func(_hero): rebuild_count += 1)
...
func test_every_rejection_leaves_xp_and_progress_unchanged() -> void:
	...
	var tree := catalog.get_role("gun")
	tree.get_node("gun.root").effect._is_valid = false
	var invalid := _hero()
	var invalid_before := _snapshot(invalid)
	_assert_rejected_without_mutation(_service().purchase_node(invalid, "gun", "gun.root"), ProgressionPurchaseResult.Status.INVALID_EFFECT, invalid, invalid_before)
